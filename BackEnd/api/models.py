...
BRANCH_DEPARTMENT_MAPPING = {
    v: k for k, v in DEPARTMENT_BRANCH_MAPPING.items()
}
...
def validate_usn_format(usn, department):
    ...
    if not match:
        return (
            False,
            "Invalid USN format. Expected format: 2GI22XX001 where XX is branch code and last 3 digits are 001+",
            None,
        )
    ...
    if branch_code != expected_branch:
        return (
            False,
            f"USN branch code '{branch_code}' does not match selected department '{department}' (expected: {expected_branch})",
            None,
        )
...
def validate_employee_id_format(employee_id, department):
    ...
    if not match:
        return (
            False,
            "Invalid Employee ID format. Expected format: XXYYY where XX is branch code and YYY is number",
        )
    ...
    if branch_code != expected_branch:
        return (
            False,
            f"Employee ID branch code '{branch_code}' does not match selected department '{department}' (expected: {expected_branch})",
        )
...
class User(AbstractUser):
    ...
    signature = models.ImageField(
        ...
        blank=True,
        null=True,
        help_text="Digital signature for certificate signing (JPEG/PNG)",
    )
...
class Student(models.Model):
    ...
    admission_type = models.CharField(
        max_length=10, choices=ADMISSION_TYPE_CHOICES, default="regular"
    )
    phone_number = models.CharField(max_length=15, blank=True, null=True)
    date_of_birth = models.DateField(blank=True, null=True)
    address = models.TextField(blank=True, null=True)
    profile_photo = models.ImageField(
        upload_to="profile_photos/students/", blank=True, null=True
    )
    ...
    def __str__(self):
        return (
            f"{self.user.get_full_name() or self.user.username} ({self.usn})"
        )

    @property
    def total_aicte_points(self):
        return (
            self.aicte_transactions.filter(status="APPROVED").aggregate(
                total=models.Sum("points_allocated")
            )["total"]
            or 0
        )
...
class Mentor(models.Model):
    ...
    address = models.TextField(blank=True, null=True)
    profile_photo = models.ImageField(
        upload_to="profile_photos/mentors/", blank=True, null=True
    )
    qualifications = models.TextField(blank=True, null=True)
    bio = models.TextField(blank=True, null=True)
    signature = models.ImageField(
        upload_to="signatures/mentors/",
        blank=True,
        null=True,
        help_text="Digital signature for certificate signing (JPEG/PNG)",
    )
...
class ClubOrganizer(models.Model):
    ...
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="club_organizer_profile"
    )
    ...
    address = models.TextField(blank=True, null=True)
    profile_photo = models.ImageField(
        upload_to="profile_photos/club_organizers/", blank=True, null=True
    )
    designation_in_club = models.CharField(
        max_length=100, blank=True, null=True
    )
    bio = models.TextField(blank=True, null=True)
    profile_completed = models.BooleanField(default=False)
...
class ClubMember(models.Model):
    ...
    student = models.ForeignKey(
        Student, on_delete=models.CASCADE, related_name="club_memberships"
    )
...
class HallBooking(models.Model):
    ...
    event = models.ForeignKey(
        "Event", on_delete=models.CASCADE, related_name="hall_bookings"
    )
    booked_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="hall_bookings"
    )
...
class Event(models.Model):
    ...
    end_date = models.DateField(
        blank=True,
        null=True,
        help_text="For multi-day events. If null, event is single-day",
    )
    ...
    awardsAictePoints = models.BooleanField(
        default=False, help_text="Whether event awards AICTE points"
    )
    ...
    points_awarded = models.IntegerField(
        default=0, help_text="Points awarded per attendance"
    )

    # Hall Assignment Fields
    needsVenueCampus = models.BooleanField(
        default=False, help_text="Whether event requires college campus venue"
    )
    primary_hall = models.ForeignKey(
        ...
        help_text="Preferred hall for the event",
    )
    secondary_hall = models.ForeignKey(
        Hall,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="secondary_events",
        help_text="Backup hall for the event",
    )
    assigned_hall = models.ForeignKey(
        ...
        help_text="Currently assigned hall",
    )
    hall_assigned_at = models.DateTimeField(
        null=True, blank=True, help_text="When hall was last assigned"
    )
    ...
    def assign_hall(self):
        ...
        if self.secondary_hall and self._is_hall_available(
            self.secondary_hall
        ):
...
class EventRegistration(models.Model):
    ...
    student = models.ForeignKey(
        Student, on_delete=models.CASCADE, related_name="event_registrations"
    )
...
class CertificateTemplate(models.Model):
    name = models.CharField(
        max_length=100, default="Global Certificate Template"
    )
    template_file = models.FileField(
        upload_to="certificate_templates/",
        help_text="Global certificate template (HTML/PDF)",
    )
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True
    )
...
class PrincipalSignature(models.Model):
    ...
    notes = models.TextField(
        blank=True, null=True, help_text="Notes about the signature upload"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_active:
            PrincipalSignature.objects.filter(is_active=True).exclude(
                pk=self.pk
...
class Certificate(models.Model):
    ...
    file_hash = models.CharField(
        ...
        editable=False,
        null=True,
        help_text="SHA256 of the certificate file for verification.",
    )
...
class AICTECategory(models.Model):
    ...
    max_points_allowed = models.IntegerField(
        default=20,
        help_text="Maximum points that can be awarded (default 20 per VTU rules)",
    )

    def clean(self):
        if (self.min_points_required is not None) and (
            self.max_points_allowed is not None
        ):
            if self.min_points_required > self.max_points_allowed:
                raise ValidationError(
                    "min_points_required cannot be greater than max_points_allowed"
                )
...
class AICTEPointTransaction(models.Model):
    ...
    student = models.ForeignKey(
        Student, on_delete=models.CASCADE, related_name="aicte_transactions"
    )
    event = models.ForeignKey(
        Event, on_delete=models.CASCADE, related_name="aicte_transactions"
    )
    category = models.ForeignKey(
        AICTECategory, on_delete=models.CASCADE, related_name="transactions"
    )
    ...
    verification_code = models.CharField(
        ...
        blank=True,
        null=True,
        help_text="8-character verification code for mentor approval",
    )
    status = models.CharField(
    ...
    def clean(self):
        if self.category:
            ...
            if minp is not None and self.points_allocated < minp:
                raise ValidationError(
                    f"points_allocated ({self.points_allocated}) is below category minimum ({minp})."
                )
            if maxp is not None and self.points_allocated > maxp:
                raise ValidationError(
                    f"points_allocated ({self.points_allocated}) exceeds category maximum ({maxp})."
                )
...
class Notification(models.Model):
    ...
    notification_type = models.CharField(
        max_length=30, choices=NOTIFICATION_TYPES, default="info"
    )
    ...
    aiCTE_transaction = models.ForeignKey(
    ...
    hall_booking = models.ForeignKey(
        "HallBooking", on_delete=models.SET_NULL, null=True, blank=True
    )
